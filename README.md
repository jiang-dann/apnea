...
* **重採樣：** 將所有資料重新採樣成相同頻率，以便後續處理及訓練。
* **雜訊濾除：** 處理環境與硬體造成的生理訊號雜訊干擾，確保特徵乾淨。
* **標準化：** 為解決因設備、人種、性別所產生的力道影響，透過標準化的方式，將其數值拉到一個較為平均的範圍。
* **資料不平衡：** 針對病患與健康樣本比例懸殊的問題，實作有效的資料擴增與重採樣策略，防止模型產生偏見。

### 3. 模型訓練與優化
* 透過反覆的邏輯拆解與參數調校，建構穩定的預測模型。
